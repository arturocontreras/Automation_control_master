...
function tacoma_rk(inter,ic,n,p,w,omega,d)
...
for k=1:n
    for i=1:p
        ...
        y(i+1,:)=runge_step(t(i),y(i,:),h,w,omega,d);
        yy(5*k+i-5)= y(1,1);
        theta(5*k+i-5) = y(1,3);
    end
    y(1,:)=y(p+1,:);t(1)=t(p+1);
...
function y=trapstep(t,x,h,w,omega,d)
%one step of the Trapezoid Method
z1=ydot(t,x,w,omega,d);
g=x+h*z1;
z2=ydot(t+h,g,w,omega,d);
y=x+h*(z1+z2)/2;

function y=runge_step(t,x,h,w,omega,d)

   k1 = ydot(t,x,w,omega,d);
   g1 = x + 0.5*h*k1;

   k2 = ydot(t+0.5*h,g1,w,omega,d);
   g2 = x + 0.5*h*k2;
   
   k3 = ydot(t+0.5*h,g2,w,omega,d);
   g3 = x + h*k3;
   
   k4 = ydot(t+h,g3,w,omega,d);
   
   y = x + (h/6)*(k1 + 2*k2 + 2*k3 + k4);

function ydot=ydot(t,y,w,omega,d)
len=6;a=0.2; W=w;
