...
c = 450;
m = 10;
Fseca =  1*400;   % Variar el coeficente de 0 a 2.75 
...
k = 1;
for tt = ti:dt:tf
  y = x + 0.05*randn(1,1);%salida x
  
  pos(k,1)    = x;
  vel(k,1)    = xp;
  Preio(k,1)   = Pipo;
  
  xxo1(k,1) = xo(1,1);   xxo2(k,1) = xo(2,1);   xxo3(k,1) = xo(3,1)/z;  %Estados del observ
   
  error = x - xast(k,1);
  xsp = -K*[ error; xo(2,1);  z*(xo(3,1))];
