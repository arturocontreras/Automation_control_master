...
Fric = 1*10;                    %Fricción seca externa
...
Wfi = [ 0
        0       
        0 
       -1];                   %Fricción seca actúa como una perturbación no lineal
Ci = [ 1  0  0  0];
Di = [0];
Q  = [q];
...
for tt = ti:dt:tf                 %Tiempo de la simulación
    ...
    time(k,1) = tt;               %Tiempo para graficar
    ui = -Kfbi(1,1:3)*xi -Kfbi(1,4)*int_err - Kffi*xmi;  %señal de control feedback + feedforward
                                                         %con integrador
%    if(tt > 5)                   % Analizar efecto cuando se corta voltaje 
%        u = 0;
%    end
    if(ui > umax)                  %Limitamos la señal de control (voltaje)
       ui = umax;
    elseif(ui < -umax)
       ui = -umax;
    end
    ...
    uci(k,1) = ui;                %Para graficar la señal de control
